...
<div class="page-heading mb-4">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h3 class="mb-1 text-primary fw-bold">Data Anggota</h3>
            <p class="text-subtitle text-muted">Kelola data anggota organisasi.</p>
        </div>
    </div>
</div>
    ...
    <div class="card card-modern">
        <div class="card-header card-header-modern">
            <div class="d-flex align-items-center gap-2">
                ...
                <span class="fw-bold fs-5">Daftar Anggota</span>
            </div>
            <div>
                ...
                <a href="<?= base_url('admin/anggota/new') ?>" class="btn btn-add-new">
                    ...
                    <tbody>
                        <?php foreach($anggota as $key => $a): ?>
                            <tr>
                                ...
                                <td>
                                    <img src="<?= base_url('uploads/foto_anggota/' . ($a['foto'] ? $a['foto'] : 'default.jpg')) ?>" class="user-avatar" alt="Foto">
                                </td>
                                <td class="fw-bold text-dark"><?= esc($a['nama_lengkap']) ?></td>
                                <td><span class="badge bg-light-primary text-primary"><?= esc($a['nama_rt']) ?></span></td>
                                <td class="text-center">
                                    <button onclick="showQrModal('<?= base_url('uploads/qr/' . $a['qr_code']) ?>', '<?= esc($a['nama_lengkap']) ?>')" class="btn-action btn-qr" title="Lihat QR">
                                        <i class="bi bi-qr-code"></i>
                                    </button>
                                </td>
                                <td class="text-center">
                                    <a href="<?= base_url('admin/anggota/edit/' . $a['id']) ?>" class="btn-action btn-edit me-1" title="Edit Data">
                                        <i class="bi bi-pencil-fill"></i>
                                    </a>
                                    <button onclick="confirmDelete(<?= $a['id'] ?>)" class="btn-action btn-delete" title="Hapus Data">
                                        <i class="bi bi-trash-fill"></i>
                                    </button>
                                    <form id="delete-form-<?= $a['id'] ?>" action="<?= base_url('admin/anggota/delete/' . $a['id']) ?>" method="get" class="d-none"></form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog">
        <form action="<?= base_url('admin/anggota/import') ?>" method="post" enctype="multipart/form-data" class="modal-content">
            <?= csrf_field() ?>
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title text-white">Import Data Anggota</h5>
            ...
            <div class="modal-body">
                ...
                <a href="<?= base_url('admin/anggota/download-template') ?>" class="btn btn-sm btn-outline-primary w-100">
...
<script>
    ...
    function confirmDelete(id) {
        Swal.fire({
            title: 'Hapus Data?', text: "Data anggota yang dihapus tidak dapat dikembalikan!", icon: 'warning',
