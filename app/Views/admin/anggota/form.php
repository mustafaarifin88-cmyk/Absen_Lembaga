...
<div class="page-heading mb-4">
    <div class="d-flex align-items-center">
        <a href="<?= base_url('admin/anggota') ?>" class="btn btn-light rounded-circle me-3 shadow-sm" style="width: 45px; height: 45px; display: flex; align-items: center; justify-content: center;">
            <i class="bi bi-arrow-left text-primary fs-5"></i>
        </a>
        <div>
            <h3 class="mb-0 fw-bold text-primary"><?= isset($anggota) ? 'Edit Data Anggota' : 'Tambah Anggota Baru' ?></h3>
            <p class="text-muted mb-0">Lengkapi formulir di bawah ini dengan data yang valid.</p>
        </div>
    </div>
</div>
        ...
        <div class="col-lg-8">
            <div class="card card-form">
                <div class="card-header card-header-gradient">
                    <h5 class="mb-0 text-white"><i class="bi bi-person-badge me-2"></i> Informasi Anggota</h5>
                </div>
                <div class="card-body p-4 p-md-5">
                    
                    <form action="<?= isset($anggota) ? base_url('admin/anggota/update/' . $anggota['id']) : base_url('admin/anggota/create') ?>" method="post" enctype="multipart/form-data">
                        <?= csrf_field() ?>
                        ...
                        <div class="row g-4">
                            <div class="col-12 text-center mb-2">
                                ...
                                <div class="upload-area" onclick="document.getElementById('foto').click()">
                                    ...
                                    <div id="upload-placeholder" class="<?= isset($anggota) && $anggota['foto'] ? 'd-none' : '' ?>">
                                    ...
                                    <div id="preview-container" class="<?= isset($anggota) && $anggota['foto'] ? '' : 'd-none' ?>">
                                        <img id="preview-img" src="<?= isset($anggota) ? base_url('uploads/foto_anggota/' . $anggota['foto']) : '' ?>" class="preview-img mb-2">
                                        <p class="mb-0 small text-primary fw-bold" id="preview-text">
                                            <?= isset($anggota) ? 'Klik untuk ubah foto' : 'Foto terpilih' ?>
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div class="col-12">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control form-control-modern form-control-modern-group <?= $validation->hasError('nama_lengkap') ? 'is-invalid' : '' ?>" 
                                           ...
                                           value="<?= old('nama_lengkap', isset($anggota['nama_lengkap']) ? $anggota['nama_lengkap'] : '') ?>">
                            ...
                            <div class="col-12">
                                <label for="rt_id" class="form-label-custom">RT (Rukun Tetangga)</label>
                                <div class="input-group">
                                    <span class="input-group-text input-group-text-modern"><i class="bi bi-house-fill"></i></span>
                                    <select class="form-select form-control-modern form-control-modern-group <?= $validation->hasError('rt_id') ? 'is-invalid' : '' ?>" id="rt_id" name="rt_id">
                                        <option value="" disabled selected>-- Pilih RT --</option>
                                        <?php foreach($rt as $r): ?>
                                            <option value="<?= $r['id'] ?>" <?= old('rt_id', isset($anggota['rt_id']) ? $anggota['rt_id'] : '') == $r['id'] ? 'selected' : '' ?>>
                                                <?= $r['nama_rt'] ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                    <div class="invalid-feedback ps-2">
                                        <?= $validation->getError('rt_id') ?>
                                    </div>
                                </div>
                            </div>

                            <div class="col-12 mt-5 d-flex gap-2 justify-content-end">
                                <a href="<?= base_url('admin/anggota') ?>" class="btn btn-cancel">Batal</a>
